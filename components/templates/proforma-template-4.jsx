...
const RepeatingHeader = () => (
  <div className="flex justify-between items-start pb-2 border-b-2 border-green-600 bg-white">
    <div className="flex items-center space-x-4">
      <Image src="/images/logo3.png" alt="Mars Logo" width={60} height={60} />
      <div>
        <h1 className="font-bold text-base text-green-900">MARS RETAIL TRADE OF</h1>
        <h1 className="font-bold text-base text-green-900">CONSTRUCTION MATERIALS</h1>
      </div>
    </div>
    <div className="text-right">
      <h2 className="text-lg font-bold text-green-600">PROFORMA</h2>
      <h2 className="text-lg font-bold text-green-600">INVOICE</h2>
    </div>
  </div>
)
...
const CustomerDetailsSection = ({ data }) => (
  <section className="my-6 flex justify-between text-xs">
    <div>
      <p className="font-bold text-green-700">Invoice to:</p>
...
const ProformaTemplate4 = ({ data, notValid }) => {
  ...
  return (
    <>
      {paginatedItems.map((pageItems, pageIndex) => (
        <div
          key={pageIndex}
          className="invoice-page bg-white shadow-lg w-[595px] h-[842px] p-8 flex flex-col mb-8 text-green-900 relative overflow-hidden"
        >
          {/* Watermark */}
          {notValid && (
            <div
                  ...
                  {pageItems.map((item, index) => {
                    ...
                    return (
                      <tr key={index} className="border-b border-green-200">
                        ...
                        <td className="p-2 text-right text-xs">{formatNumber(unitPriceWithoutTax)}</td>
                        <td className="p-2 text-right text-xs">{formatNumber(unitPriceWithoutTax * (item.qty || 0))}</td>
                      </tr>
                    )
                  })}
                </tbody>
                {pageIndex === paginatedItems.length - 1 && (
                  <tfoot>
                    <tr className="border-t-2 border-green-400">
                      ...
                      <td className="p-2 text-right text-xs">{formatNumber(subtotal)}</td>
                    </tr>
                    <tr>
                      <td colSpan="4" />
                      <td className="p-2 text-right text-xs font-bold">VAT ({taxRate.toFixed(2)}%)</td>
                      <td className="p-2 text-right text-xs">{formatNumber(totalTax)}</td>
                    </tr>
                    <tr className="bg-green-900 text-white">
                      <td colSpan="4" />
                      <td className="p-2 text-right text-xs font-bold">Grand Total</td>
                      <td className="p-2 text-right text-xs font-bold">{formatNumber(total)}</td>
                    </tr>
                  </tfoot>
                )}
              </table>
      ...
      {showFinalSectionsOnNewPage && (
        <div className="invoice-page bg-white shadow-lg w-[595px] h-[842px] p-8 flex flex-col mb-8 text-green-900 relative overflow-hidden">
          {/* Watermark */}
          {notValid && (
            <div
              className="pointer-events-none select-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2"
...
export default ProformaTemplate4
